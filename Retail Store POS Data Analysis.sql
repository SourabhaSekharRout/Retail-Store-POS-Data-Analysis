--Creating database RETAIL_SALES_DATA
...
FROM 'C:\Users\soura\OneDrive\Desktop\SQL case studies\SQL Case Study - Basic\Customer.csv'
WITH(
FIRSTROW = 2 ,
FIELDTERMINATOR = ',' ,
ROWTERMINATOR = '\n' ,
...
FROM 'C:\Users\soura\OneDrive\Desktop\SQL case studies\SQL Case Study - Basic\prod_cat_info.csv'
WITH (
FIRSTROW = 2 ,
FIELDTERMINATOR = ',' ,
ROWTERMINATOR = '\n' 
...
FROM 'C:\Users\soura\OneDrive\Desktop\SQL case studies\SQL Case Study - Basic\Transactions.csv'
WITH (
FIRSTROW = 2 ,
FIELDTERMINATOR = ',' ,
ROWTERMINATOR = '\n' ,
...
SELECT TOP 1 store_type AS MOST_USED_CHANNEL , COUNT(*) AS TOTAL_TRANSACTION
FROM Transactions
GROUP BY store_type
...
SELECT COUNT(DISTINCT(prod_subcat_code)) AS SUBCATEGORIES_OF_BOOK
FROM Prod_cat_info
...
ROUND(SUM( Transactions.total_amt),2) AS NET_TOTAL_REVENUE
FROM Transactions
...
FROM(
SELECT Transactions.cust_id , COUNT( DISTINCT ( Transactions.transaction_id )) AS TC
FROM Transactions
WHERE total_amt > 0 
GROUP BY cust_id
HAVING COUNT ( DISTINCT (Transactions.transaction_id)) > 10 ) AS SUBQUERY ;
...
JOIN Prod_cat_info ON Prod_cat_info.prod_cat_code = Transactions.prod_cat_code AND
Transactions.prod_subcat_code = Prod_cat_info.prod_subcat_code
...
FROM Transactions 
JOIN Prod_cat_info ON Transactions.prod_cat_code = Prod_cat_info.prod_cat_code 
AND Transactions.prod_subcat_code = Prod_cat_info.prod_subcat_code
...
FROM Transactions
JOIN Prod_cat_info ON Transactions.prod_cat_code = Prod_cat_info.prod_cat_code
WHERE Qty<0 AND tran_date BETWEEN (SELECT DATEADD(MONTH, -3,(SELECT MAX( tran_date )))
